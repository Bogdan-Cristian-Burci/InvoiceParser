...
# Conditional import for LLMWhisperer
try:
    from .llmwhisperer_extractor import LLMWhispererExtractor
...
class TableExtractor:
    ...
    def __init__(self, config: ProcessingConfig):
        self.config = config
        self.llmwhisperer_extractor = None

        # Initialize LLMWhisperer if enabled and API key is provided
        if self.config.use_llmwhisperer:
            if not LLMWHISPERER_AVAILABLE or LLMWhispererExtractor is None:
                logger.warning("LLMWhisperer requested but not available - falling back to standard extraction")
            elif not self.config.llmwhisperer_api_key:
                logger.warning("LLMWhisperer requested but no API key provided - falling back to standard extraction")
            else:
                try:
                    self.llmwhisperer_extractor = LLMWhispererExtractor(self.config.llmwhisperer_api_key)
                    logger.info("LLMWhisperer extractor initialized successfully")
                except Exception as e:
                    logger.warning(f"Failed to initialize LLMWhisperer: {e}")
                    if not self.config.llmwhisperer_fallback:
                        raise
    ...
    def extract_page_data(self, pdf_path: str, page_number: int) -> PageData:
        ...
        try:
            # Extract raw text from page - use LLMWhisperer if available and enabled
            page_data.raw_text = self._extract_page_text(pdf_path, page_number)
    ...
    def _extract_page_text(self, pdf_path: str, page_number: int) -> str:
        """
        Extract text from a specific page using LLMWhisperer if available,
        ...
        """
        # Try LLMWhisperer first if enabled and available
        if self.llmwhisperer_extractor and self.config.use_llmwhisperer:
            try:
                logger.debug(f"Using LLMWhisperer to extract text from page {page_number + 1}")

                # Extract specific page using LLMWhisperer
                pages_to_extract = [page_number + 1]  # LLMWhisperer uses 1-indexed pages
                result = self.llmwhisperer_extractor.extract_specific_pages(
                    pdf_path,
                    pages_to_extract,
                    mode=self.config.llmwhisperer_mode
                )

                if result['success'] and result['extracted_text']:
                    logger.info(f"LLMWhisperer successfully extracted text from page {page_number + 1}")
                    return result['extracted_text']
                else:
                    logger.warning(f"LLMWhisperer failed to extract text from page {page_number + 1}: {result.get('error', 'Unknown error')}")

                    # Fall back to standard extraction if fallback is enabled
                    if self.config.llmwhisperer_fallback:
                ...
                logger.error(f"Error using LLMWhisperer for page {page_number + 1}: {e}")

                # Fall back to standard extraction if fallback is enabled
                if self.config.llmwhisperer_fallback:
                    logger.info("Falling back to standard text extraction due to LLMWhisperer error")
                else:
                    # If no fallback, return empty string
                    return ""

        # Use standard extraction (fallback or default)
        try:
            logger.debug(f"Using standard text extraction for page {page_number + 1}")
            return extract_text_from_page(pdf_path, page_number)
        except Exception as e:
            logger.error(f"Standard text extraction failed for page {page_number + 1}: {e}")
            return ""
